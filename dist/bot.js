...
let lastMessageTime = null; // Last message sent time
...
const isHoliday = (date) => {
    ...
    return config_1.holidays.some((holiday) => holiday.date === today);
...
const getRandomCooldownMinutes = () => {
    return Math.floor(Math.random() * (60 - 40 + 1)) + 40; // Random between 40 and 60
};
const sendMessage = (channel) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // }
    const sentMessage = yield channel.send(randomMessage);
...
const isWithinTimeRange = (now) => {
    return true;
    const estOffset = now.getTimezoneOffset() + (new Date().getTimezoneOffset() - 300);
...
const scheduleMessages = () => {
    const job = node_schedule_1.default.scheduleJob("*/30 * * * * *", () => __awaiter(void 0, void 0, void 0, function* () {
        const now = new Date();
        ...
        //isCooldownOver(now)
            ...
            console.log(`Skipping message at ${now}: Not a weekday, it's a holiday, or outside of time range`);
        }
    }));
};
// Bot ready event
